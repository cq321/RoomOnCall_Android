...
import com.onjection.handler.Urls;
...
public class ServerDownload extends AsyncTask<String, Void, JSONObject> {
	...
	protected JSONObject doInBackground(String... params) {
		...
		if (params[0] == null) {
			...
			if (tagResult.equals("login")) {
				getJSONFromUrl("http://www.roomoncall.in/api/login",
						params[0], HttpMethod.POST);
			} else if (tagResult.equals("register")) {
				getJSONFromUrl("http://www.roomoncall.in/api/registration",
						params[0], HttpMethod.POST);
			} else if (tagResult.equals("reservation")) {
				getJSONFromUrl("http://www.roomoncall.in/api/reservation",
						params[0], HttpMethod.POST);
			} else if (tagResult.equals("confirm_reservation")) {
				getJSONFromUrl(
						"http://www.roomoncall.in/api/confirm_reservation",
						params[0], HttpMethod.POST);
			} else if (tagResult.equals("bookingDetails")) {
				getJSONFromUrl("http://www.roomoncall.in/api/usersbooking",
						params[0], HttpMethod.POST);
			}else if (tagResult.equals("contact")) {
			    getJSONFromUrl(Urls.ContactUsApi, params[0], HttpMethod.POST);
			   }
	...
	protected void onPostExecute(JSONObject result) {
		...
		if (new ConnectionDector(mContext).isNetError) {
			Utilz.showAlertActivityWithTitle("Sorry", "slow internet", mContext);
		}else {
